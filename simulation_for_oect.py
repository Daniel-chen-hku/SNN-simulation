...
def snn_learn(trainingset,teacherset,testset,answer,stdp_data):
    if 9 != trainingset.shape[1] or 3 != teacherset.shape[1]:
    ...
    for i in range(trainingset.shape[0]):
        stdp(trainingset[i].tolist(),teacherset[i].tolist(),stdp_data)
        oect_list.append(copy.deepcopy(stdp_data.g_oect))
...
if __name__ == '__main__':
    ...
    datasource = 'create' if len(sys.argv) <= 1 else float(sys.argv[1])
    assert datasource == 'create' or datasource == 'load'
    if datasource == 'load':
        trainingset = np.load('trainingset.npy')
        teacherset = np.load('teacherset.npy')
        ...
        np.save('teacherset.npy',teacherset)
